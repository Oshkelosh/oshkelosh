...
def template_route(file_name = None):
    ...
    path = os.path.join(site_config['style'], style_config['template_path'], file_name if file_name else '')
    return path
...
def static_route(file_name = None):
    ...
    site_config = json.loads(current_app.redis.get('site_config'))
    style_config = json.loads(current_app.redis.get(f'{site_config['style']}_style'))
    path = os.path.join('addons', 'styles', site_config['style'], style_config['static_path'], file_name if file_name else '')
    return path
