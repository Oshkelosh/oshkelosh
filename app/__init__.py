...
def create_app():
    ...
    if 'FLASK_ENV' not in env_config:
        print("Please set FLASK_ENV in the .env file. Possible environments: 'development', 'production', 'testing', 'default'")
        return False
    ...
    success = migrations.check_defaults(db_path=schema.db_path, defaults=default.default_list)
    if not success:
        return False
    ...
    path = os.path.join(app.root_path, 'addons','styles')
